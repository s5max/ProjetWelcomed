	...
	<div class="card-container manual-flip">
		<div class="card">
			<div class="front">
				...
				<div class="content">
					<div class="main">
						<p class="text-center"><strong><?= $v['kind'].' : '.$v['type'] ?></strong><br>Date de publication :<?= $v['date_create']?> <br><?= $d['department']?>, <?= $v['city']?><br><?= $d['content']?></p>
					</div>
...
<!--						<p>Heures d'activités : <?=$d['opening'].' à '.$d['closing']?></p>-->
						<p class="text-center"><?= $d['content']?><br>
						Visite : <?php if(isset($d['office']) && $d['office'] == 'on'){ echo'Cabinet'; } if(isset($d['home']) && $d['office'] == 'on'){ echo'Domicile'; } ?>
						Domicile/Cabinet<br></p>
<!--						<?= $d['exercise']?><br>Nbre de praticiens : <?= $d['nbPraticioner']?><br>Logiciel utilisé : <?=$d['software']?></p>-->
					...
					<?php if(!isset($_SESSION['user'])){ ?>
						...
						<div id="ad_action">
						<a id="contact" class="btn btn-lg btn-rounded btn-primary waves-effect waves-light btn-contact" data-receiver="<?=$v['user_id'];?>" disabled>Contacter l'annonceur</a>
						</div>
						<div id="no-log" class="col-sm-12">
						
							<a class="btn btn-rounded btn-primary mod" data-url="<?= $m ?>subscribe.php" data-toggle="modal" data-target="#modal4all">S'inscrire</a>

							<a class="btn btn-rounded btn-primary mod" data-url="<?= $m ?>login.php" data-toggle="modal" data-target="#modal4all">Se connecter</a>
					...
					<div id="ad_action">
					<a id="contact" class="btn btn-lg btn-rounded btn-primary waves-effect waves-light btn-contact mod" data-url="<?= $m ?>contact_advertiser.php" data-receiver="<?=$v['user_id'];?>" data-toggle="modal" data-target="#modal4all">Contacter l'annonceur</a>
					</div>

...
if(!empty($_POST)){
	...
	if(isset($_FILES['cv']) && $_FILES['cv']['error'] === 0){
		...
		if(in_array($mimeType, $mimeTypeAvailable)){
			...
			if($_FILES['cv']['size'] <= $maxSize){
				...
				if(!move_uploaded_file($_FILES['cv']['tmp_name'], $uploadDir.$newCvName)){
					$error['cv'] = '<p class="error">Erreur lors de l\'upload de fichier</p>';
				}else{
					$cv = 'ok';
				}
			}
		...
		$error['cv'] = '<p class="error">Le fichier ne correspond pas aux critères de taille</p>';
		...
		if(count($error) === 0){
			...
			$insert->bindValue(':sender_id',$_SESSION['user']['id']);
...
<!--                            </div>-->
	<form id="advertiser" method="post">
	<input type="hidden" id="sender_id" name="sender_id" class="form-control" value="<?= $_SESSION['user']['id'] ?>">
		<?php 	if(isset($post['receiver_id'])){
					echo '<input type="hidden" id="receiver_id" name="receiver_id" class="form-control" value="'. $post['receiver_id'].'">';	
				}else{
					echo '<input type="hidden" id="receiver_id" name="receiver_id" class="form-control" value="'.$_POST['receiver'].'">';
